...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const minScore = envFloat("RAG_MIN_SCORE", 0.2);
    ...
    if (!hasEvidence) {
      const { data: q } = await supabase
        ...
        .single();
      return Response.json(
        { status: "insufficient_evidence", answer: NO_ANSWER, citations: [], queryId: q?.id },
        { headers: corsHeaders },
      );
    }
    ...
    const context = selected
      .map(
        (x, i) =>
          `[SOURCE ${i + 1} | chunk_id=${x.chunk_id} | ${x.title} | ${x.section_path ?? ""}]\n${x.content}`,
      )
      .join("\n\n");
    ...
    const citations = selected.map((x) => ({
      chunkId: x.chunk_id,
      documentId: x.document_id,
      title: x.title,
      section: x.section_path,
    ...
    return Response.json(
      { status: "answered", answer, citations, queryId: q?.id },
      { headers: corsHeaders },
    );
  } catch (error) {
